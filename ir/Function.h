...
public:
    ...
    LocalVariable * newLocalVarValue(Type * type, std::string name = "", int32_t scope_level = 1, bool is_const = false);

    /// @brief 新建局部数组变量
    /// @param name 数组名
    /// @param type 数组类型
    /// @return 局部数组变量
    LocalVariable * createLocalArray(std::string name, Type * type);
    ...
    ///
    /// @brief 循环标签栈，用于break和continue语句
    ///
    struct LoopLabels {
        LabelInstruction * startLabel;
        LabelInstruction * endLabel;
    };
    std::vector<LoopLabels> loopLabels;
    ...
    ///
    void pushLoopLabels(LabelInstruction * startLabel, LabelInstruction * endLabel);
    ...
    /// @return 循环开始标签
    ///
    LabelInstruction * getCurrentLoopStartLabel();

    ///
    /// @brief 获取当前循环的结束标签
    /// @return 循环结束标签
    ///
    LabelInstruction * getCurrentLoopEndLabel();
