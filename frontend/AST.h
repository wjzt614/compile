...
enum class ast_operator_type : int {
    ...
    /// @brief 数组形参运算符，属性包含名字与类型，以及数组维度
    ...
    /// @brief 数组初始化列表
    AST_OP_ARRAY_INIT_LIST,

    /// @brief 数组元素初始化值
    AST_OP_ARRAY_INIT_ELEM,

    /// @brief 空数组初始化
    AST_OP_ARRAY_INIT_EMPTY,
...
/// @return 维度列表
///
std::vector<uint32_t> extractArrayDimensions(ast_node * dimsNode, bool isParam);
