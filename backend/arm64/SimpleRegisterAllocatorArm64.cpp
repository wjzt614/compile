///
/// @file SimpleRegisterAllocatorArm64.cpp
/// @brief 简单或   
...
///
SimpleRegisterAllocatorArm64::SimpleRegisterAllocatorArm64()
{}
...
/// @param no 指定的寄存器编号
///
int SimpleRegisterAllocatorArm64::Allocate(Value * var, int32_t no)
{
...
/// @param no 要分配的寄存器编号
///
void SimpleRegisterAllocatorArm64::Allocate(int32_t no)
{
...
/// @param var 变量
///
void SimpleRegisterAllocatorArm64::free(Value * var)
{
    if (var && (var->getLoadRegId() != -1)) {
        // 清除该索引的寄存器，变得可使用
...
/// @param no 寄存器编号
///
void SimpleRegisterAllocatorArm64::free(int32_t no)
{
    // 无效寄存器，什么都不做，直接返回
    if (no == -1) {
        return;
...
/// @param no
///
void SimpleRegisterAllocatorArm64::bitmapSet(int32_t no)
{
    regBitmap.set(no);
    usedBitmap.set(no);
}