...
public:
    ...
    /// @brief 新建一个浮点型数值的Value，并加入到符号表，用于后续释放空间
    /// \param floatVal 浮点数值
    /// \return 临时Value
    ...
    Value * newVarValue(Type * type, std::string name = "", bool is_const = false);
    ...
    /// @return 全局数组变量
    ///
    GlobalVariable * createGlobalArray(std::string name, Type * type);
