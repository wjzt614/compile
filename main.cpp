...
{
    std::cout << exeName + " -S [--symbol] [-T | --ast | -I | --ir | -C] [-o output | --output=output] source\n";
    std::cout << "Options:\n";
    ...
    std::cout << "  -C,                        Output control flow graph\n";
...
static int ArgsAnalysis(int argc, char * argv[])
{
    ...
    const char options[] = "ho:STIO:t:cC";
    int option_index = 0;
...
static int compile(std::string inputFile, std::string outputFile)
{
    ...
    do {
        ...
        if (gOptLevel > 0) {
            ...
            // 应用死代码消除优化
            DeadCodeEliminator optimizer(module);
            optimizer.run();
        ...
        if(gShowCFG) {
            ...
            // 跳过内置函数
                ...
                optimizer.markUnreachableBlocks(func ,true);
            }
            break;
        }
        if (gShowLineIR) {
        ...
        // 后端处理，体系结果相关的操作
        // 使用面向ARM64的汇编产生器CodeGeneratorArm64
        if (gShowASM) {
